...
asn1c_save_compiled_output(arg_t *arg, const char *datadir,
	...
	if (arg->flags & A1C_MAKE) {	  
	  ...
	  if (mkdir("./cmake", S_IRUSR|S_IWUSR|S_IXUSR|S_IRGRP|S_IXGRP) < 0 && errno != EEXIST)
	    {
	      perror("Cannot create cmake directory");
	      return -1;
	    }
	  mkf = asn1c_open_file("cmake/asn1_pdus", ".cmake", 0);
	  if(mkf == NULL) {
	    perror("cmake/asn1_pdus.cmake");
	    return -1;
	  }	  
	  safe_fprintf(stderr, "CMake generation NYI\n");
	  mkf_filename = "cmake/asn1_pdus.cmake";
	...
	if (arg->flags & A1C_MAKE) {	  
	  ...
	  safe_fprintf(mkf, "\n\n"
		       "add_library(libasn1_pdus ${ASN_MODULE_SOURCES} ${ASN_MODULE_HEADERS})\n\n"
		       "target_compile_definitions(libasn1_pdus PUBLIC _DEFAULT_SOURCE)\n\n"
		       "target_include_directories(libasn1_pdus PRIVATE ${CMAKE_CURRENT_SOURCE_DIR})\n\n"
		       );
	}
	else if (arg->flags & A1C_LXMAKE) {
	}
